...
class SpeechDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # STFT
        window = torch.hann_window(self.n_fft,device=x_noisy.device)
        x_noisy_stft = torch.stft(input=x_noisy, n_fft=self.n_fft, hop_length=self.hop_length, normalized=True,return_complex=True,window=window)
        x_noisy_stft =torch.view_as_real(x_noisy_stft)
        x_clean_stft = torch.stft(input=x_clean, n_fft=self.n_fft, hop_length=self.hop_length, normalized=True,return_complex=True,window=window)
        x_clean_stft = torch.view_as_real(x_clean_stft)
        g1_stft = torch.stft(input=g1_wav, n_fft=self.n_fft, hop_length=self.hop_length, normalized=True,return_complex=True,window=window)
